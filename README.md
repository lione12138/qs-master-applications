...
| [QS 前200 扩展路线图](docs/QS200_ROADMAP.md)
...
- `data/coverage.json`
  - 从正式数据自动生成的 QS 前200入口、规则、项目和精确日期覆盖率
...
前端状态规则：开放日距离用户当天 1–30 天的窗口归为“即将开放”；
31 天及以后归为“未来开放”；开放日当天开始归为“正在开放”。预测窗口
沿用相同分组，但始终保留“非官方预测”和来源周期标识。
...
QS 1–55 已完成入口和规则核验。只有同时具备精确开放日、截止日和可复核
官网证据的记录才进入正式窗口；其余学校保留周期说明，并继续由监控任务
等待下一周期的准确日期。
